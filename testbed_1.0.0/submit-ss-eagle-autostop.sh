...
then
    ...
    awk -F"=" 'BEGIN{OFS=FS} $1=="job.id"{$2=$2+1}1' src/main/config/stock-exchange-ss-eagle.properties > properties.tmp
    mv properties.tmp src/main/config/stock-exchange-ss-eagle.properties
fi
...
if [ $job == 1 ]
then
OUTPUT=`./bin/run-app.sh --config-factory=org.apache.samza.config.factories.PropertiesConfigFactory --config-path=file://$PWD/config/stock-exchange-ss-eagle.properties | grep 'application_.*$'`
cd ..
...
#ssh eagle << EOF
    /home/samza/generate.sh eagle:9092
#EOF
