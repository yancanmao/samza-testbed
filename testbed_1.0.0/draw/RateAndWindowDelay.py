# -*- coding: utf-8 -*-
def draw(deltaT, jobname):
    #jobname = '4h_16_L5T5l120'
    retValue = []
    input_file = '/home/samza/GroundTruth/' + jobname + "/000001.txt"
    ...
    majorstep = 600
    xlabels = [''] * (14400 / minorstep)
    for x in range(0, 14400, majorstep):
    ...
    with open(input_file) as f:
        ...
        for i in range(0, len(lines)):
            ...
            if(unionBidFlag and split[0] =='State,' and split[5] == 'Arrived:'):
                ok = "".join(split[6:]).replace(' ', '').replace('{', '').replace('}', '').split(',')
                for item in ok:
                    if(int(item.split('=')[1]) > 0):
                        unionBidFlag = False
                if(not unionBidFlag):
                    print('Union bid over: ' + split[2])
            if(not unionBidFlag):
            ...
            if (split[0] == 'Migration!'):
                ...
                if(not split[j + 4].startswith('[')):
                    ...
                    srcs = [split[tt].rstrip().replace('[','').replace(']','').replace(',','') for tt in range(j + 4, j + 4 + mid)]
                    tgts = [split[tt].rstrip().replace('[','').replace(']','').replace(',','') for tt in range(j + 4 + mid + 2, len(split))]
                    time = (long(split[j + 1]) - initialTime) / base
                    for t in srcs:
            ...
            if (split[0] == 'Number' and split[2] == 'severe'):
                ...
                numberOfSevereT += [(time - initialTime) / base]
            ...
            if (split[0] == 'Executors' and split[1] == 'stopped'):
                ...
                if(not split[i+1].startswith('[')):
                    ...
                    if (',new' not in split):
                        tgts = [split[tt].rstrip().replace('[','').replace(']','').replace(',','') for tt in range(i + 1 + mid + 1, len(split))]
                    else: #Scale-out mix load-balance
                        tmid = split[mid+1:].index(',new')
                        tgts = [split[tt].rstrip().replace('[','').replace(']','').replace(',','') for tt in range(mid + 1 + tmid + 1, len(split))]
                        print('tgt=' + str(tgts))
                    srcs = [split[tt].rstrip().replace('[','').replace(']','').replace(',','') for tt in range(i + 1, i + 1 + mid)]
    ...
    if(executorsFigureFlag and utilizationFlag):
        ...
        for groupId in range(0, len(groups)):
            ...
            for Id in groups[groupId]:
                ...
                index += 1
                legend = ['Utilization']
                plt.plot(util[Id][0], util[Id][1],'r^', markersize=1)
    ...
    if(executorsFigureFlag and cpuFlag):
        ...
        for groupId in range(0, len(groups)):
            ...
            for Id in groups[groupId]:
                ...
                plt.plot(processCpuUsage[Id][0], [x / 3.125 for x in processCpuUsage[Id][1]], 'r^', markersize=1)
                plt.plot(util[Id][0], util[Id][1], 'bo', markersize=1)
    ...
    plt.plot(arrivalRateT, arrivalRate , 'r', markersize=1)
    plt.legend(legend, loc='upper left')
...
if __name__ == "__main__":
    jobname = '4h_16_L1T1l100h100d100m60000_dfactor0.875_yancanservice_noisolated_largequeue_conservative0.8_delay4ms_recoveroptimized_'
    draw(100, jobname)
