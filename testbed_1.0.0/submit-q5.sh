...
#    ~/samza-hello-samza/deploy/kafka/bin/kafka-topics.sh --zookeeper localhost:2181 --create --topic stock_sb --partitions 64 --replication-factor 1 --config message.timestamp.type=LogAppendTime
#
...
./bin/run-app.sh --config-factory=org.apache.samza.config.factories.PropertiesConfigFactory --config-path=file://$PWD/config/nexmark-q5-local.properties
cd ..
