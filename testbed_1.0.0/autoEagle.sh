...
while IFS=" " read -r nOE L T l_low l_high deltaT mInterval dFactor muFactor
do
    echo $nOE $L $T $l_low $l_high $deltaT $mInterval $dFactor $muFactor;
    #Modify applicaiton config
    cp ~/workspace/samza-related/samza-testbed-stock/testbed_1.0.0/src/main/config/stock-exchange-ss-eagle.properties properties.t1
    ...
    awk -F"=" 'BEGIN{OFS=FS} $1=="streamswitch.requirement.latency"{$2='"$((L*1000))"'}1' properties.t1 > properties.t2
    ...
    rm properties.t1
    mv properties.t2 ~/workspace/samza-related/samza-testbed-stock/testbed_1.0.0/src/main/config/stock-exchange-ss-eagle.properties
    #Get appid
    date
    echo 'Start application...'
    OUTPUT=`./submit-ss-eagle-autostop.sh 1 1 1 1`
    #./submit-ss-eagle-autostop.sh 1 1 1 1
    #echo $OUTPUT
    ...
    localDir="4h_${nOE}_L${L}T${T}l${l_low}h${l_high}d${deltaT}m${mInterval}_newapp_conservative${muFactor}_delay5000us_serializeoptimized"
...
#    echo $locaDir
#    ./runCprhost.sh eagle ${appid} ${localDir} 
done < ~/experimentlist_eagle
